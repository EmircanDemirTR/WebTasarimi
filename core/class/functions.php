...
$before = array("{","}");
$after = array('<span class="font-weight-100">','</span>');
$dataaaa = str_replace($before,$after,$data);
...
function colorTextt($data){
    
$before = array("{","}");
$after = array('<span class="font-weight-300 text-primary">','</span>');
$dataaaa = str_replace($before,$after,$data);
...
function escapeDBString($con,$data){
...
function uppercase($data){
    ...
    $before = array("a","b","c","ç","d","e","f","g","ğ","h","ı","i","j","k","l","m","n","o","ö","p","r","s","ş","t","u","ü","v","y","z","q","x","w");
    
    $after = array("A","B","C","Ç","D","E","F","G","Ğ","H","I","İ","J","K","L","M","N","O","Ö","P","R","S","Ş","T","U","Ü","V","Y","Z","Q","X","W");
...
function uppercaseEn($data){
    
    $before = array("a","b","c","ç","d","e","f","g","ğ","h","ı","i","j","k","l","m","n","o","ö","p","r","s","ş","t","u","ü","v","y","z","q","x","w");
    
    $after = array("A","B","C","Ç","D","E","F","G","Ğ","H","I","I","J","K","L","M","N","O","Ö","P","R","S","Ş","T","U","Ü","V","Y","Z","Q","X","W");
...
function getDBString($data){
    ...
    $data = preg_replace("/[^A-Z0-9_\s-]/i", "", $data);
...
function randomGenerate($data){
    ...
    return substr(md5(sha1(crc32($data))),0,15);
...
function rootPath(){
    ...
    if($getData->siteConfig["directory"] != ""){
        
        $dir = "/". $getData->siteConfig["directory"]."/". $_GET["lang"] ."/";
...
function dataStatus($data){
    ...
    switch($data){
        ...
        case "1":
            ...
            return $pageStatus = '<span class="badge badge-success" style="font-size:13px; padding:7px 15px;">Aktif</span>';
        
        break;
                
        case "0":
        
            return $pageStatus = '<span class="badge badge-danger" style="font-size:13px; padding:7px 15px;">Pasif</span>';
...
function userStatus($data){
    ...
    switch($data){
        ...
        case "1":
            ...
            return $pageStatus = '<span class="label label-success">Preminum Üye</span>';
        
        break;
                
        case "0":
        
            return $pageStatus = '<span class="label label-important">Ücretsiz Üye</span>';
...
function imgDelete($path, $data){
	
	$dir = "../core/uploads/";
	
	foreach($path as $value){
		
		if (file_exists($dir . $value . $data)){
		
			$unlink .= unlink($dir  . $value . $data);
...
function randomGenerator(){
	...
	return date("dmy") . rand();
	
}

function characterLimit($data){
	...
	if($count > $limit){
		...
		$title = mb_substr($data, 0, $limit, 'UTF-8');
...
'123' => 'application/vnd.lotus-1-2-3'
    ...
    return $mime_types[$file]; // return the array value
}
