...
<li class="nav-item"><a class="nav-link active" id="home-info" data-toggle="tab" href="#info" aria-controls="1" aria-expanded="true">Ürün Dokümanı</a></li>
</ul>
	 
	 
<form class="form-horizontal" method="post" enctype="multipart/form-data" action="main.php?module=product/pagedocument&id=<?PHP print $getData->siteConfig["getID"]; ?>"  >
	...
	if(!empty($_FILES["document"]["name"])){
		...
		if($upload->file_src_error == 0){
			...
			if ($upload->uploaded){
				...
				if ($upload->processed){
					redirect("main.php?module=product/pagedocument&id=". $getData->siteConfig["getID"] ."&success");
					$upload->clean();
				}else{
					redirect("main.php?module=product/pagedocument&id=". $getData->siteConfig["getID"] ."&success");
				}
			...
			redirect("main.php?module=product/pagedocument&id=". $getData->siteConfig["getID"] ."&errorFile");
		}
