...
<form class="form-horizontal" action="main.php?module=settings/logo&id=<?PHP print $getData->siteConfig["getID"]; ?>"  method="post" style="display:inline;">
<div class="row">
...
<div class="col-md-12">
 ...
 <?PHP print $adminEngine->siteLogoControl("0", "fff");?>
</div> 
...
if($_POST){
		...
		if($upload->file_src_error == 0){
				...
				if ($upload->processed){
					...
					foreach($data as $fieldName => $fieldData){
						...
						if(!$adminEngine->updateFieldDB("md_system", "data", $fieldData, "name", $fieldName)){
							...
							redirect("main.php?module=settings/logo&failed&" . $fieldData);
							
						}
							
					}	
					
					redirect("main.php?module=settings/logo&success");
					$upload->clean();
				}else{
					redirect("main.php?module=settings/logo&success");
				}
			...
			redirect("main.php?module=settings/logo&errorFile");
		}
