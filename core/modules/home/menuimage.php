...
<form class="form-horizontal" method="post" enctype="multipart/form-data" action="main.php?module=page/menuimage&id=<?PHP print $getData->siteConfig["getID"]; ?>"  >
	...
	if(!empty($_FILES["image"]["name"])){
		...
		if($upload->file_src_error == 0){
			if ($upload->uploaded){
				...
				if ($upload->processed){
					redirect("main.php?module=page/menuimage&id=". $getData->siteConfig["getID"] ."&success");
					$upload->clean();
				}else{
					redirect("main.php?module=page/menuimage&id=". $getData->siteConfig["getID"] ."&success");
				}
			...
			redirect("main.php?module=page/menuimage&id=". $getData->siteConfig["getID"] ."&errorFile");
		}
	}
