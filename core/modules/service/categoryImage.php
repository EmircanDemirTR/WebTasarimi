...
<li class="nav-item"><a class="nav-link active" id="home-info" data-toggle="tab" href="#info" aria-controls="1" aria-expanded="true">Kategori Kapak Görseli</a></li>
</ul>
  ...
  <div class="tab-content" id="myTabContent">
	...
	<div class="col-sm-7">
    <form action="main.php?module=product/categoryImage&id=<?PHP print $getData->siteConfig["getID"]; ?>" method="post" enctype="multipart/form-data" class="avatar">
        <label class="col-sm-6 form-control-label">Resim Yükleme Aracı</label>
        <div class="col-sm-6">
            <div class="slim" data-fetcher="fetch.php" data-force-size="700,550">
...
if($_POST){
		...
		if($upload->file_src_error == 0){
			if ($upload->uploaded){
				...
				if ($upload->processed){
					redirect("main.php?module=product/categoryImage&id=". $getData->siteConfig["getID"] ."&success");
					$upload->clean();
				}else{
					redirect("main.php?module=product/categoryImage&id=". $getData->siteConfig["getID"] ."&success");
				}
			...
			redirect("main.php?module=product/categoryImage&id=". $getData->siteConfig["getID"] ."&errorFile");
		}
