...
<li class="nav-item"><a class="nav-link active" id="home-info" data-toggle="tab" href="#info" aria-controls="1" aria-expanded="true">Sayfa Arkaplan Resmi</a></li>
</ul>
	 
	 
<form class="form-horizontal" method="post" enctype="multipart/form-data" action="main.php?module=page/bgimage&id=<?PHP print $getData->siteConfig["getID"]; ?>"  >
  <div class="tab-content" id="myTabContent">
	...
	<div class="col-sm-5">
		<?PHP print $adminEngine->pageBgImgControl();?>
    </div>
...
if($_FILES){
	if(!empty($_FILES["image"]["name"])){
		...
		if($upload->file_src_error == 0){
			if ($upload->uploaded){
				...
				$upload->file_new_name_body = 'bg_'.$getData->siteConfig["getID"];
				...
				$adminEngine->pagethumb($_FILES['image'], "bg_".$getData->siteConfig["getID"]);
				if ($upload->processed){
					redirect("main.php?module=page/bgimage&id=". $getData->siteConfig["getID"] ."&success");
					$upload->clean();
				}else{
					redirect("main.php?module=page/bgimage&id=". $getData->siteConfig["getID"] ."&success");
				}
			...
			redirect("main.php?module=page/bgimage&id=". $getData->siteConfig["getID"] ."&errorFile");
		}
	}
	...
	$file = "bg_".$getData->siteConfig["getID"] . ".jpg";
