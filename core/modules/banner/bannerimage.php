...
<li class="nav-item"><a class="nav-link active" id="home-info" data-toggle="tab" href="#info" aria-controls="1" aria-expanded="true">Banner Arkaplan Resmi</a></li>
</ul>
  ...
  <div class="tab-content" id="myTabContent">
	...
	<div class="col-sm-7">
    <form action="main.php?module=banner/bannerimage&id=<?PHP print $getData->siteConfig["getID"]; ?>" method="post" enctype="multipart/form-data" class="avatar">
        <label class="col-sm-6 form-control-label">Resim Yükleme Aracı</label>
        <div class="col-sm-6">
            <div class="slim" data-fetcher="fetch.php" data-force-size="1920,736">
	...
	<div class="col-sm-5">
		<?PHP print $adminEngine->bannerBgImgControl();?>
    </div>
...
if($_POST){
		...
		if($upload->file_src_error == 0){
			if ($upload->uploaded){
				...
				$upload->file_new_name_body = 'bbg_'.$getData->siteConfig["getID"];
				...
				$adminEngine->helloThumb($write_dir.$temp_code, "bbg_".$getData->siteConfig["getID"], $thumbDir);
				if ($upload->processed){
					redirect("main.php?module=banner/bannerimage&id=". $getData->siteConfig["getID"] ."&success");
					$upload->clean();
				}else{
					redirect("main.php?module=banner/bannerimage&id=". $getData->siteConfig["getID"] ."&success");
				}
			...
			redirect("main.php?module=banner/bannerimage&id=". $getData->siteConfig["getID"] ."&errorFile");
		}
	...
	$file = "bbg_".$getData->siteConfig["getID"] . ".jpg";
